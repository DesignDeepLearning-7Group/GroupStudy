...
# 결측값 가진 열과 , 개수 확인
result = has_null[has_null > 0].to_dict()
# print(result)
...
# 전력 열 숫자로 바꾸기 , 결측으로 변환
df["전력"] = pd.to_numeric(df["전력"], errors="coerce")

null_counts = df["전력"].isnull().sum()
# print(null_counts)
...
# 지우고 표 크기 출력
after_deleting = df.drop_duplicates()
# print(after_deleting.shape)

# 인덱스 다시
after_deleting = after_deleting.reset_index(drop=True)
...
cpu_maen = df["CPU온도"].mean()
memory_median = df["메모리"].median()
power = df["전력"].mean()

# 평균등으로 채우기
after_deleting["CPU온도"] = after_deleting["CPU온도"].fillna(cpu_maen)
after_deleting["메모리"] = after_deleting["메모리"].fillna(memory_median)
after_deleting["전력"] = after_deleting["전력"].fillna(power)

# 채운 후 결측값 확인
# print(after_deleting.duplicated().sum())

# print(round(cpu_maen, 2), memory_median)
...
# 지표 4종 평균 출력
# print(
#     after_deleting.groupby("구역")[["CPU온도", "전력", "응답시간", "메모리"]]
#     .mean()
#     .round(2)
# )


# 구역별 수집 건수 를 구역 이름으로 출력
